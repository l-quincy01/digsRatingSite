...
$images = "SELECT * FROM thecreatorscollective.images WHERE Digs_ID = $selectedDigsID ; ";
$result = mysqli_query($conn, $digs) or die("Query error");
$result2 = mysqli_query($conn, $images) or die("Query error");
        ...
        if ($result->num_rows > 0) {
          ...
          $counter = 0; 
            while ($row = $result->fetch_assoc()) {
                ...
                // Start a new flex container after every 4 items
          if($counter != 0 && $counter % 4 == 0) {
                echo '</div><div class="flex-container">';
              }
               
                echo  ' <div class="property-card"  id="property1"  >';
                echo '<div class="comment-cell">' .'<strong>'. "Name: " .'</strong>'. $row["Tenant_Fname"] . "<br>" .'</div>';
                echo '<div class="comment-cell">' .'<strong>'. "Comment: " .'</strong>' . $row["tenant_comment"] . "<br>" .'</div>';
                
                // Display the star rating for cleanliness
                echo '<strong>'."Cleanliness: ".'</strong>' ;
